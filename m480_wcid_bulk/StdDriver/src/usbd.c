...
void USBD_ProcessSetupPacket(void)
{
    ...
    for (i = 0; i < 8 ; i++) {
        printf("0x%02X ", g_usbd_SetupPacket[i]);
    }
    printf("\r\n");
...
void USBD_GetDescriptor(void)
{
    ...
    case DESC_STRING:
    {
        ...
        if(g_usbd_SetupPacket[2] < 4ul)
        {
            ...
            break;
        } else if (g_usbd_SetupPacket[2] == 0xEE){  // OS String 
            printf("OS String\r\n");
            u32Len = g_usbd_sInfo->gu8StringDesc[4][0];
            USBD_PrepareCtrlIn((uint8_t *)g_usbd_sInfo->gu8StringDesc[4], u32Len);
            break;
        }
        else
        {
